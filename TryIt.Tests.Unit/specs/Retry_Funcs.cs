...
namespace TryIt.Tests.Unit.specs
{
    class Retry_Funcs : nspec
    {
        void with_no_arguments()
        {
            ...
            act = () => subject = Retry.TryIt.Try(subjectFunc, retries);
            ...
            describe["TryIt.Try(func, retries).WithSuccessPolicy()"] = () =>
            {
                ...
                describe["TryIt.Try(func, retries).WithSuccessPolicy(SuccessPolicyDelegate<T>).Go()"] = () =>
                {
                    ...
                    act = () =>
                    {
                        subject = Retry.TryIt.Try(subjectFunc, retries);
                        try
                        {
            ...
            describe["TryIt.Try(func, retries).Go()"] = () =>
            {
                ...
                {
                    Action action = () => Retry.TryIt.Try(subjectFunc, 0);
        ...
        void with_1_argument()
        {
            ...
            act = () => subject = Retry.TryIt.Try(subjectFunc, arg, retries);
            ...
            describe["TryIt.Try(func, retries).ThenTry(altFunc, retries)"] = () =>
            {
                ...
                act = () =>
                {
                    subject = Retry.TryIt.Try(subjectFunc, arg, retries);
        ...
        void with_2_arguments()
        {
            ...
            act = () => subject = Retry.TryIt.Try(subjectFunc, arg1, arg2, retries);
        ...
        void with_3_arguments()
        {
            ...
            act = () => subject = Retry.TryIt.Try(subjectFunc, arg1, arg2, arg3, retries);
        ...
        void with_4_arguments()
        {
            ...
            act = () => subject = Retry.TryIt.Try(subjectFunc, arg1, arg2, arg3, arg4, retries);
