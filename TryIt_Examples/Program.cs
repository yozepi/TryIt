...
namespace TryIt_Examples
{
    class Program
    {
        ...
        static async Task<string> DownloadStringAsync(string url)
        {
            using (var request = new WebClient())
            {
                var response = await request.DownloadStringTaskAsync(url);
                return response;
            }
        }
            ...
            try
            {
                ...
                result = TryIt.Try(() => GetDBResults(connA), 3)
                    ...
                    .ThenTry(() => GetDBResults(connA), 6)
                      .UsingFibonacciDelay(TimeSpan.FromMilliseconds(100))
        ...
        static async Task Try_Method_Quick_Then_BackOff_Async()
        {
            ...
            try
            {
                ...
                result = await TryIt.TryAsync(() => GetDBResults(connA), 3)
                    ...
                    //...then try connA 6 times with using a back-off delay that starts at 100ms...
                    .ThenTry(() => GetDBResults(connA), 6)
                      .UsingBackoffDelay(TimeSpan.FromMilliseconds(100))
